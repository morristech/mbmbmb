...
public class NavigationView extends ScrimInsetsFrameLayout {
    ...
    class C06761 implements Callback {
    ...
    public static class SavedState extends AbsSavedState {
        ...
        static class C00451 implements ClassLoaderCreator<SavedState> {
            C00451() {
            }
    ...
    public NavigationView(Context context, AttributeSet attributeSet, int i) {
        ...
        attributeSet = TintTypedArray.obtainStyledAttributes(context, attributeSet, C0022R.styleable.NavigationView, i, C0022R.style.Widget_Design_NavigationView);
        ViewCompat.setBackground(this, attributeSet.getDrawable(C0022R.styleable.NavigationView_android_background));
        if (attributeSet.hasValue(C0022R.styleable.NavigationView_elevation) != 0) {
            ViewCompat.setElevation(this, (float) attributeSet.getDimensionPixelSize(C0022R.styleable.NavigationView_elevation, 0));
        }
        ViewCompat.setFitsSystemWindows(this, attributeSet.getBoolean(C0022R.styleable.NavigationView_android_fitsSystemWindows, false));
        this.mMaxWidth = attributeSet.getDimensionPixelSize(C0022R.styleable.NavigationView_android_maxWidth, 0);
        if (attributeSet.hasValue(C0022R.styleable.NavigationView_itemIconTint) != 0) {
            i = attributeSet.getColorStateList(C0022R.styleable.NavigationView_itemIconTint);
        } else {
            i = createDefaultColorStateList(16842808);
        }
        if (attributeSet.hasValue(C0022R.styleable.NavigationView_itemTextAppearance)) {
            resourceId = attributeSet.getResourceId(C0022R.styleable.NavigationView_itemTextAppearance, 0);
            i2 = 1;
        } else {
            i2 = false;
            resourceId = i2;
        ...
        if (attributeSet.hasValue(C0022R.styleable.NavigationView_itemTextColor)) {
            colorStateList = attributeSet.getColorStateList(C0022R.styleable.NavigationView_itemTextColor);
        }
        if (i2 == 0 && r4 == null) {
            colorStateList = createDefaultColorStateList(16842806);
        }
        Drawable drawable = attributeSet.getDrawable(C0022R.styleable.NavigationView_itemBackground);
        this.mMenu.setCallback(new C06761());
        ...
        if (attributeSet.hasValue(C0022R.styleable.NavigationView_menu) != null) {
            inflateMenu(attributeSet.getResourceId(C0022R.styleable.NavigationView_menu, 0));
        }
        if (attributeSet.hasValue(C0022R.styleable.NavigationView_headerLayout) != null) {
            inflateHeaderView(attributeSet.getResourceId(C0022R.styleable.NavigationView_headerLayout, 0));
        }
    ...
    private ColorStateList createDefaultColorStateList(int i) {
        ...
        if (!getContext().getTheme().resolveAttribute(C0248R.attr.colorPrimary, typedValue, true)) {
            return null;
        }
